...
class DataRequestOutput(BaseModel):
    time_series_data: pd.DataFrame = Field(
        ..., description="dataframe of synthetic data"
    )
...
def generate_synthetic_data(request: DataRequestInput) -> DataRequestOutput:
    ...
    df["coal_price"] = (
        2
        + 0.3 * np.sin(2 * np.pi * df["month"] / 12)
        + np.random.normal(0, 0.05, len(df))
    )

    # Weather conditions
    df["temperature"] = (
        15 + 10 * np.sin(2 * np.pi * df["month"] / 12) + np.random.normal(0, 3, len(df))
    )
    df["humidity"] = (
        60 + 20 * np.sin(2 * np.pi * df["month"] / 12) + np.random.normal(0, 5, len(df))
    )
    df["wind_speed"] = (
        5 + 2 * np.sin(2 * np.pi * df["hour"] / 24) + np.random.normal(0, 1, len(df))
    )
...
def save_data_locally(data: DataRequestOutput, directory: str = "data/raw") -> None:
    ...
    file_name = os.path.join(directory, "synthetic_data.csv")
