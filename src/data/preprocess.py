...
class Dataset(BaseModel):
    time_series_data: pd.DataFrame = Field(
        ..., description="dataframe of imported data"
    )
...
class PreprocessConfig(BaseModel):
    ...
    validation_ratio: float = Field(
        default=0.2, description="validation data proportion"
    )
...
def load_data(config: PreprocessConfig) -> Dataset:
    ...
    for file in data_dir.glob("*.csv"):
        if not file:
            raise ValueError("No csv file found during preprocess loading")
        file_name = file.stem
...
def split_data(
    ...
    validation = dataset.time_series_data.iloc[
        training_len : training_len + validation_len, :
    ]
...
def preprocess_data(config: PreprocessConfig) -> Tuple[Dataset, Dataset, Dataset]:
