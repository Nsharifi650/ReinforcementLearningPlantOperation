...
class AgentConfig(BaseModel):
    ...
    environment: DummyVecEnv = Field(
        ..., description="Environment for the agent to run"
    )
    validation_timesteps: int = Field(
        ..., description="Number of steps for the validation"
    )
    train_timesteps: int = Field(
        default=100, description="ONLY FOR TESTING!! USED TO UPLOAD SAVED MODEL"
    )
...
class PPOAgent:
    def __init__(self, config: AgentConfig):
        ...
        self.Agent = PPO(
            "MlpPolicy", config.environment, verbose=1, learning_rate=1e-5, gamma=0.99
        )  # batch_size=256
    ...
    def train(self):
        self.Agent.learn(
            total_timesteps=self.config.total_timesteps, callback=self.callback
        )
    ...
    def validate(self):
        ...
        for ii in range(self.config.validation_timesteps):
            ...
            observation, reward, done, _ = environment.step(
                actions
            )  # Unpacking only 4, even though 5 is returned - issue with current version of DummyVecEnv
            ...
            if done:
                observation = environment.reset()
        print(
            f"Accumulative Reward at the end of {self.config.validation_timesteps} steps validation: {accumulative_reward[-1]}"
        )
        logger.info(
            f"Accumulative Reward at the end of {self.config.validation_timesteps} steps validation: {accumulative_reward[-1]}"
        )
